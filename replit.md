...
✓ **PostgreSQL Database Connected**: Uses Replit's built-in PostgreSQL database via Drizzle ORM + `drizzle-orm/node-postgres` (pg driver). All tables migrated (users, clients, social_accounts, bots, bot_templates, analytics, scheduled_posts, drafts). Credentials are auto-managed via DATABASE_URL secret. Users and data persist across server restarts.

**Note:** `DATABASE_URL` is auto-managed by Replit's PostgreSQL integration. Do not manually override it in the Secrets panel — the connection string is provisioned and maintained automatically. The `server/db.ts` connection code prefers `DATABASE_URL` over individual `PG*` environment variables.
...
### Database Schema (Drizzle ORM - Replit PostgreSQL)
...
- Register/login routes write and read from the database
...
1. **User Authentication**: JWT-based authentication with PostgreSQL storage
